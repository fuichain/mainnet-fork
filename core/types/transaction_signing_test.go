...
func TestEIP155Signing(t *testing.T) {
	...
	signer := NewEIP155Signer(func(blockNumber *big.Int) *big.Int {
		return big.NewInt(18)
	})
	tx, err := SignTx(NewTransaction(0, addr, new(big.Int), 0, new(big.Int), nil), signer, new(big.Int), key)
	if err != nil {
		t.Fatal(err)
	}

	from, err := Sender(signer, tx, new(big.Int))
	if err != nil {
		t.Fatal(err)
	}
	if from != addr {
...
func TestEIP155ChainId(t *testing.T) {
	...
	if tx.ChainId().Cmp(signer.chainId(new(big.Int))) != 0 {
		t.Error("expected chainId to be", signer.chainId(new(big.Int)), "got", tx.ChainId())
	}

	tx = NewTransaction(0, addr, new(big.Int), 0, new(big.Int), nil)
	tx, err = SignTx(tx, HomesteadSigner{}, new(big.Int), key)
	if err != nil {
		t.Fatal(err)
	}
...
func TestChainId(t *testing.T) {
	...
	var err error
	tx, err = SignTx(tx, NewEIP155Signer(func(blockNumber *big.Int) *big.Int {
		return big.NewInt(1)
	}), new(big.Int), key)
	if err != nil {
		t.Fatal(err)
	...
	_, err = Sender(NewEIP155Signer(func(blockNumber *big.Int) *big.Int {
		return big.NewInt(2)
	}), tx, new(big.Int))
