...
type driver interface {
	...
	SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int, blockNumber *big.Int) (common.Address, *types.Transaction, error)
...
func (w *wallet) SignTx(account accounts.Account, tx *types.Transaction, chainID *big.Int, blockNumber *big.Int) (*types.Transaction, error) {
	...
	sender, signed, err := w.driver.SignTx(path, tx, chainID, blockNumber)
	if err != nil {
		return nil, err
	}
