...
func generatePreMergeChain(n int) (*core.Genesis, []*types.Block) {
	...
	generate := func(i int, g *core.BlockGen) {
		...
		tx, _ := types.SignTx(types.NewTransaction(testNonce, common.HexToAddress("0x9a9070028361F7AAbeB3f2F2Dc07F82C4a98A02a"), big.NewInt(1), params.TxGas, big.NewInt(params.InitialBaseFee*2), nil), types.LatestSigner(config, new(big.Int)), new(big.Int), testKey)
		g.AddTx(tx)
		testNonce++
	}
...
func TestEth2AssembleBlock(t *testing.T) {
	...
	signer := types.NewEIP155Signer(func(blockNumber *big.Int) *big.Int {
		return ethservice.BlockChain().Config().ChainID
	})
	tx, err := types.SignTx(types.NewTransaction(uint64(10), blocks[9].Coinbase(), big.NewInt(1000), params.TxGas, big.NewInt(params.InitialBaseFee), nil), signer, big.NewInt(2), testKey)
	if err != nil {
...
func TestEth2NewBlock(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		tx, _ := types.SignTx(types.NewContractCreation(nonce, new(big.Int), 1000000, big.NewInt(2*params.InitialBaseFee), logCode), types.LatestSigner(ethservice.BlockChain().Config(), big.NewInt(2)), big.NewInt(2), testKey)
		ethservice.TxPool().AddLocal(tx)
