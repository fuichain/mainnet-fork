...
func makechain() (bc *core.BlockChain, addrHashes, txHashes []common.Hash) {
	...
	blocks, _ := core.GenerateChain(gspec.Config, genesis, ethash.NewFaker(), db, testChainLen,
		func(i int, gen *core.BlockGen) {
			...
			nonce := uint64(i)
			if i%4 == 0 {
				tx, _ = types.SignTx(types.NewContractCreation(nonce, big.NewInt(0), 200000, big.NewInt(0), testContractCode), signer, new(big.Int), bankKey)
				addr = crypto.CreateAddress(bankAddr, nonce)
			} else {
				addr = common.BigToAddress(big.NewInt(int64(i)))
				tx, _ = types.SignTx(types.NewTransaction(nonce, addr, big.NewInt(10000), params.TxGas, big.NewInt(params.GWei), nil), signer, new(big.Int), bankKey)
			}
			gen.AddTx(tx)
	...
	for !f.exhausted {
		...
		case 6:
			...
			for i := range req.Txs {
				...
				req.Txs[i], _ = types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(10000), params.TxGas, big.NewInt(1000000000*int64(f.randomByte())), nil), signer, new(big.Int), bankKey)
			}
